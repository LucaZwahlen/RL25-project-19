...
class GaussianThompsonSampling:
    ...
    def select_param(self):
        eps = 1e-6
        samples = [np.random.normal(m, max(s, eps)) for m, s in zip(self.means, self.stds)]
    ...
    def update_distribution(self, returns_tensor):
        ...
        if self.counts[i] > 1:
            self.stds[i] = np.sqrt(((self.stds[i] ** 2) * (self.counts[i] - 1) + (reward - prev_mean) * (reward - self.means[i])) / self.counts[i])
