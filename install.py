...
def check_python_version():
    ...
    if major != 3 or minor != 10:
        print("This script requires Python 3.10.x. Please install the correct version and try again.")
        sys.exit(1)
...
def check_cuda():
    ...
    try:
        ...
        result = subprocess.run(['nvidia-smi', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = result.stdout.decode('utf-8')
        cuda_string = [line for line in output.split('\n') if 'CUDA Version' in line][0]
        cuda_version = re.search(r'\s*CUDA Version\s*:\s*(\d+\.\d+)', cuda_string).group(1)
...
def get_cuda_index_url(cuda_version):
    ...
    print("CUDA version not supported for PyTorch installation. Make sure you have CUDA 12.6 or higher.")
    return None
