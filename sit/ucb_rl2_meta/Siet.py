                ...
                for j in range(0, dim):
                    ...
                    if (i, j) == (i0, j0):
                        ...
                        try:
                            ...
                            angle1 = (
                                torch.acos(
                                    (vec / torch.norm(vec)) @ (vecf / torch.norm(vecf))
                                )
                                * 1
                                / math.pi
                            )  # maximal 1
                            angle2 = (
                                torch.acos(
                                    (vec_i / torch.norm(vec_i))
                                    @ (vecf / torch.norm(vecf))
                                )
                                * 1
                                / math.pi
                            )
...
class Rotation_Symmetry_Break(nn.Module):
    ...
    def forward(self, x):
        ...
        angles = torch.index_select(
            self.angles, 1, self.idxs_angles.flatten().to(x.device)
        )
        ...
        # print("angles shape", angles.shape)
        y = (
            (y * angles.view(1, heads, 3 * self.num_patches**2))
            .reshape((bs, heads, ps, ps, 3))
            .sum(-1)
        )
...
def grid_to_idx(dim, features):
    ...
    add = (
        torch.arange(0, dimu * features, dimu)
        .reshape(features, 1)
        .repeat(1, n**2)
        .reshape(features, n, n)
    )
...
class Weighted_Symmetry_Break(nn.Module):
    ...
    def forward(self, x):
        ...
        y = (
            self.triangle_sum(y[:, 1:])
            .reshape(bs, heads, heads, ps - 1)
            .transpose(-2, -1)
        )
        y = torch.cat([x[:, :, 0:1, :], y], dim=-2)
        # p1d = (1, 0, 1 ,0) # pad last and second to last dim by 1
...
class Sym_Break_Linear(nn.Module):
    ...
    def forward(self, x):
        ...
        W = torch.index_select(
            self.weights, 1, self.idxsW.flatten().to(x.device)
        ).reshape((self.in_features, 1, self.kernel_size, self.kernel_size))
        y = F.conv2d(
            x[:, 1:].transpose(1, 3).reshape(bs * hd, h, self.idim, self.idim),
            W,
...
class Sym_Break_Had(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat(
            [x[:, :, :, 0:1], torch.cat([x[:, :, 0:1, 1:], y], dim=2)], dim=-1
        )
...
class Sym_Break_Linear_base(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.conv2d(
            x[:, 1:]
            .transpose(1, 2)
            .reshape(bs, c, self.idim, self.idim)
            .transpose(2, 3),
            W,
            padding=self.padding,
            groups=c,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.cat(
            [self.projf(nn.GELU()(y)), self.projf2(nn.GELU()(x[:, 0]))], dim=-1
        )
        return y  #
...
class Siet(nn.Module):
    def __init__(
        ...
        self.graph_block = Sym_Break_Linear_Block(
            img_size=img_size,
        ...
        self.vit_global = VisionTransformer(
            num_patches=self.num_patches,
            in_chans=embed_dim,
            embed_dim=embed_dim,
            depth=depth,
        ...
        self.patch_idxs = self.create_patching_idxs(
            dim=img_size, ps=patch_size_local, ps2=patch_size
        )  # .flatten()
    ...
    def create_patching_idxs(self, dim=32, ps=3 * 4, ps2=4):
        ...
        arr = (
            torch.tensor(list(range(image_dim_pad**2)))
            .reshape((image_dim_pad, image_dim_pad))
            .long()
        )
    ...
    def get_patches_mini(self, x):
        ...
        x = torch.cat(
            [x[:, ::2, ::2], x[:, 1::2, 1::2], x[:, 1::2, ::2], x[:, ::2, 1::2]], dim=-1
        )
        y = F.pad(
            x.permute(0, 3, 1, 2),
        ...
        return y.permute(0, 2, 3, 4, 5, 1).reshape(
            (-1, self.patch_size_l * self.patch_size_l, 4 * c)
        )  #
...
class Sym_Break_Linear_Block(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_features, num_patches, num_heads):
        ...
        idxsW0, dimW0 = grid_dist_to_idx(
            dim=self.kernel_size0
        )  # int(math.sqrt(num_patches)))
        self.idxsW0 = idxsW0
        self.dimW0 = dimW0
        idxsW, dimW = grid_dist_to_idx(
        ...
        idxsW2, dimW2 = grid_dist_to_idx2(
        ...
        self.attn = Attention_Basic(
            in_features, num_patches=num_patches, num_heads=num_heads, qkv_bias=False
        )
        self.attn2 = Attention_Basic(
            in_features, num_patches=num_patches, num_heads=num_heads, qkv_bias=False
        )
    ...
    def forward(self, x):
        ...
        W0 = torch.index_select(
            self.weights0, 1, self.idxsW0.flatten().to(x.device)
        ).reshape((self.in_features, 1, self.kernel_size0, self.kernel_size0))
        ...
        W1 = torch.index_select(
            self.weights1, 1, self.idxsW.flatten().to(x.device)
        ).reshape((self.in_features, 1, self.kernel_size, self.kernel_size))

        Wa = torch.index_select(
            self.weightsa, 1, self.idxsW.flatten().to(x.device)
        ).reshape((self.in_features, 1, self.kernel_size, self.kernel_size))
        W1a = torch.index_select(
            self.weights1a, 1, self.idxsW.flatten().to(x.device)
        ).reshape((self.in_features, 1, self.kernel_size, self.kernel_size))

        W2 = (
            torch.index_select(self.weights2, 1, self.idxsW2.flatten().to(x.device))
            .reshape((1, self.in_features, (self.patch_size // 2) ** 2))
            .permute(0, 2, 1)
        )

        #   print("x.size()",x.size())
        y = F.conv2d(
            x.reshape(bs, f, self.idim, self.idim), W0, padding=self.padding0, groups=f
        )
        ...
        q = F.conv2d(
            x.reshape(bs, f, self.idim // 2, self.idim // 2),
            W,
            padding=self.padding,
            groups=f,
        )
        ...
        x = self.reconstruct_image(
            x.reshape(
                bs,
                ((self.idim // 2) // (self.patch_size // 2)) ** 2,
                (self.patch_size // 2) ** 2,
                f,
            ),
            ps=self.patch_size // 2,
        ...
        x_inv = self.projfinv(
            (nn.GELU()(x) * W2).sum(1)
        )  # alternatie to using token embeding, jsut multiply with weights of symmetries of graph
