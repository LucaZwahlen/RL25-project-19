...
def evaluate(args, actor_critic, device, num_processes=64, aug_id=None):
    ...
    test_envs = make_procgen_env(test_args, full_distribution=False, normalize_reward=False, rand_seed=test_args.seed, render=False,
                                 distribution_mode=test_args.distribution_mode, num_levels_override=TEST_ENV_RANGE - 1000, start_level_override=1000)

    episodeQueueCalculator = EpisodeQueueCalculator('test', test_args.seed, False, 0, test_args.env_id, test_args.num_envs, test_args.distribution_mode, device)
