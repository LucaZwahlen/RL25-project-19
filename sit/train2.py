...
parser.add_argument("--device_id", type=int, default=0, help="device")

parser.add_argument(
    "--choice", type=int, default=0, help="whihc sit model to use 0 SiT, 1 for SiTs"
)

parser.add_argument("--aug_choice", type=int, default=0, help="which aug-type selected")

parser.add_argument("--use_sit", default=True, help="use Sit model")

parser.add_argument("--use_ppo", default=False, help="use PPo algo")
...
def train(args):
    ...
    device = torch.device(
        "cpu" if not torch.cuda.is_available() else "cuda:" + str(args.device_id)
    )
    ...
    for j in trange(init_epoch, num_updates):
        ...
        for step in range(args.num_steps):
            ...
            episodeQueueCalculator.update(
                action.squeeze(-1), reward.to(device).squeeze(-1)
            )
        ...
        eval_interval = (
            max(1, num_iterations // args.n_datapoints_csv)
            if args.n_datapoints_csv
            else 1
        )
        do_eval = (
            num_iterations == 0 or (j % eval_interval == 0) or (j == num_iterations)
        )
        if do_eval:
        ...
        # Save Model
        if (
            j > 0 and j % args.save_interval == 0 or j == num_updates - 1
        ) and args.save_dir != "":
            try:
